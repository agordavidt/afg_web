...
<div class="article-hero">
    <div class="container">
        ...
        <div class="article-meta-row">
            <span class="label" style="color:var(--ink-muted);">{{ $article->category_label }}</span>
        ...
        <h1 class="display-md reveal" style="padding-bottom:4rem; max-width:84rem;">{{ $article->title }}</h1>
    </div>
</div>

@if ($article->featured_image_url)
<div style="width:100%; aspect-ratio:21/9; overflow:hidden; border-bottom:1px solid var(--border);">
...
<div class="container">
    <div class="article-layout">
        ...
        {{-- Body — TipTap HTML rendered with prose styles --}}
        ...
        <aside class="article-sidebar">
            <div class="sidebar-card--yellow" style="padding:2.4rem; border-radius:var(--radius); margin-bottom:2rem;">
                <p class="sidebar-card__title" style="margin-bottom:0.8rem;">New opportunities weekly</p>
            ...
            <div style="border:1px solid var(--border); padding:2.4rem; border-radius:var(--radius);">
                <p class="toc__title" style="margin-bottom:1.4rem;">Related articles</p>
                <div style="display:flex; flex-direction:column; gap:1.4rem;">
                    @foreach ($related as $r)
                    <a href="{{ route('articles.show', $r->slug) }}"
                       style="display:block; font-size:1.4rem; font-weight:500; color:var(--ink); line-height:1.4; transition:color 0.2s;"
                       onmouseover="this.style.color='var(--ink-muted)'" onmouseout="this.style.color='var(--ink)'">
                        {{ $r->title }} →
                    </a>
                    @endforeach
                </div>
...
<section class="section">
    <div class="container">
        <div class="section-header">
            <div class="section-meta"><span class="section-num">—</span><span class="section-tag">Keep reading</span></div>
            <a href="{{ route('articles.index') }}" class="btn--ghost-muted">View all →</a>
        </div>
        <h2 class="display-sm reveal mb-lg">More guides.</h2>
